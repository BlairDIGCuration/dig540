...
while($data_row = fgetcsv($file_handle)){
    ...
    //header in $first_line[$1] will correspond with the data in $data_row[$i].
    for($i=1; $i<count($data_row); $i++){
        ...
        //4 or 5.
        if($i<4){
            // $i starts at 1 and increased by 1 each loop, so $i can only be 1, 2, or 3 in this branch
            ...
            for($j=0; $j<count($genres); $j++){
                //We have a problem though. $j is staring at 0. Computers may count from 0, but humans 
    ...
    //... and here'd the closing </p> tag that I promised above
    print_r('</p>');
}
